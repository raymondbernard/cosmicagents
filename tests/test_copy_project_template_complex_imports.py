...
def test_copy_template_rewrites_complex_import_patterns(tmp_path: Path) -> None:
    ...
    (template_dir / "src" / "my_project_template" / "__init__.py").write_text(
        "from .main import run as launch\nfrom .module import helper as alias_helper\n",
        encoding="utf-8",
    )
    (template_dir / "src" / "my_project_template" / "main.py").write_text(
        "from src.my_project_template.module import helper\nfrom .module import helper as h\n",
        encoding="utf-8",
    )
    ...
    main_content = (target_dir / "src" / "my_new_project" / "main.py").read_text(encoding="utf-8")
    assert "from .main import run as launch" in init_content
    assert "from .module import helper as alias_helper" in init_content
    assert "from src.my_new_project.module import helper" in main_content
    assert "from .module import helper as h" in main_content
