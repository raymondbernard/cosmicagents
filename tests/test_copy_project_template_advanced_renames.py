...
def test_copy_template_renames_package_folders_and_entrypoints(tmp_path: Path) -> None:
    ...
    (template_dir / "my_project_template.py").write_text(
        "from src.my_project_template import run\n",
        encoding="utf-8",
    )
    ...
    assert (target_dir / "src" / "my_new_project").exists()
    assert (target_dir / "src" / "my_new_project" / "__init__.py").exists()
    assert (target_dir / "src" / "my_new_project" / "main.py").exists()
    assert (target_dir / "my_new_project.py").exists()
